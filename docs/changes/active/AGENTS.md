# Active Change Records

## Purpose

Owns the single authoritative, resumable plan for each material change currently being implemented.

## Local Contracts

- Keep one phase `in-progress` and record verification evidence before marking a phase done.
- Update the record before implementation work that changes phase status.
- Do not put credentials or downloaded content archives in this folder.

## Active records

- `CHG-001-wordpress-to-astro.md` — Phase 4 (DNS cutover) in progress.
- `CHG-002-editorial-redesign.md` — presentation redesign: search-first homepage, modern typography, dark/light. Verified locally; awaiting deploy.
- `CHG-004-homepage-performance-and-feed-chrome.md` — Phase 4 verified locally (no idle search-index prefetch; llms.txt published).
